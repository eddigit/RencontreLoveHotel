...
export function ModerationAvatar({ name, src, className }: {
  ...
  return (
    <Avatar className={cn('h-12 w-12 border border-white/15 bg-[#3b1749]', className)}>
      ...
      <AvatarFallback className='overflow-hidden bg-[#3b1749] p-0'>
        <img src='/default-member-couple.jpg' alt={`Avatar natif de ${name || 'membre'}`} className='h-full w-full object-cover' />
      </AvatarFallback>
    </Avatar>
  )
}
