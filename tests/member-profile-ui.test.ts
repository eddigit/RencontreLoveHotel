...
describe('member profile UI', () => {
  it('uses a compact responsive profile layout with honest member data', () => {
    ...
    expect(page).toContain("data-testid='profile-action-panel'")
    expect(page).toContain('userProfileData.photos.find')
    expect(page).toContain('Vidéo de présentation')
    expect(page).toContain("profile.intro_video_url")
    expect(page).toContain('À propos')
    expect(page).toContain('Galerie')
    expect(page).toContain('profile.bio &&')
    expect(page).toContain('@/lib/default-member-image')
    expect(page).not.toContain('Ce profil n’a pas encore ajouté de présentation.')
